...
class StartPage:
    def __init__(self, parent_window):
        ...
        self.window.geometry('500x450+500+300')

        # 加载图片
        img2 = tk.PhotoImage(file='picture/work2.jpeg', )
        label_img2 = tk.Label(self.window, image=img2)
        label_img2.place(x=0, y=0, width=692, height=461)
        img1 = tk.PhotoImage(file='picture/work.png', )
        label_img1 = tk.Label(self.window, image=img1)
        label_img1.pack()

        # 加载文字
        label = Label(self.window, text="学生信息管理系统", font=("黑体", 20))
        label.pack()
        label2 = Label(self.window, text="Creator:向振羽", font=("楷书", 10))
        label2.pack()
        label3 = Label(self.window, text="(欢迎使用本系统)", font=("黑体", 10))
        label3.pack(pady=35)
...
class AdminPage:
    def __init__(self, parent_window):
        ...
        self.window.geometry('420x400+500+300')

        #   标题
        label = tk.Label(self.window, text='管理员登陆', bg='white', font=('Verdana', 20), height=2, pady=10)
        label.grid(row=0, column=1)

        #   账号密码
        Label(self.window, text='管理员账号：', font=tkFont.Font(size=11)).grid(row=1, sticky=W, padx=10, pady=10)
        Label(self.window, text='管理员密码：', font=tkFont.Font(size=11)).grid(row=2, sticky=W, padx=10, pady=10)
        self.admin_username = tk.Entry(self.window)
        self.admin_username.grid(row=1, column=1, pady=10)
        self.admin_pass = tk.Entry(self.window, show='*')  # 密码隐藏
        self.admin_pass.grid(row=2, column=1, pady=10)

        # 加载图片
        photo1 = PhotoImage(file="picture/work4.png")
        Label(self.window, image=photo1).grid(row=1, column=2, rowspan=2, padx=5, pady=5)

        # 加载按钮，跳转login还是back
        Button(self.window, text="登陆", width=10, font=tkFont.Font(size=15), command=self.login).grid(row=3, column=0,
                                                                                                       pady=20,
                                                                                                       columnspan=2)
        Button(self.window, text="返回首页", width=10, font=tkFont.Font(size=15), command=self.back).grid(row=3,
                                                                                                          column=1,
                                                                                                          pady=20,
                                                                                                          columnspan=2)
    ...
    def login(self):
        ...
        # 数据库操作 查询管理员表
        db = pymysql.connect(host='localhost', port=3306, db='student2', user='root', password='123456')  # 打开数据库连接
        cursor = db.cursor()  # 使用cursor()方法获取操作游标
        sql = "SELECT * FROM admin_login_k WHERE admin_id = '%s'" % (
            self.admin_username.get())  # SQL 查询语句（格式化输入username）
        # 执行SQL语句
        try:
            cursor.execute(sql)
            # 获取所有记录列表
            ...
            for row in results:
                admin_id = row[0]
                admin_pass = row[1]
                print("admin_id=%s,admin_pass=%s" % (admin_id, admin_pass))  # 打印结果（格式化输出admin_id, admin_pass）
        except:
            # 查询出错
...
class StudentPage:
    def __init__(self, parent_window):
        ...
        # 加载文字、账号和密码
        label = tk.Label(self.window, text='学生登陆', bg='white', font=('Verdana', 20), height=2, pady=10)
        label.grid(row=0, column=1)
        Label(self.window, text='学生账号：', font=tkFont.Font(size=11)).grid(row=1, sticky=W, padx=10, pady=10)
        Label(self.window, text='学生密码：', font=tkFont.Font(size=11)).grid(row=2, sticky=W, padx=10, pady=10)
        self.student_id = tk.Entry(self.window)
        self.student_id.grid(row=1, column=1, pady=10)
        self.student_pass = tk.Entry(self.window, show='*')
        self.student_pass.grid(row=2, column=1, pady=10)
...
class AdminManage:
    def __init__(self, parent_window):
        ...
        # 加载图片
        global img
        img = tk.PhotoImage(file='picture/work5.png', )
        label_img = tk.Label(self.window, image=img)
        label_img.place(x=0, y=0, width=1000, height=1000)

        self.columns = ("学号", "姓名", "性别", "年龄")  # 栏目4个
        # self.columns = ("系名/学号", "姓名", "性别", "年龄")
        self.tree = ttk.Treeview(self.frame_center, show="headings", height=18, columns=self.columns)  # 使用Treeview创建表格
        self.vbar = ttk.Scrollbar(self.frame_center, orient=VERTICAL, command=self.tree.yview)  # 设置滚动条
        # 定义树形结构与滚动条
        ...
        # self.tree.column("系名/学号", width=100, anchor='center')
        ...
        # 打开数据库连接
        db = pymysql.connect(host='localhost', port=3306, db='student2', user='root', password='123456')
        cursor = db.cursor()  # 使用cursor()方法获取操作游标
        ...
        self.right_top_button4 = ttk.Button(self.frame_right_top, text='查询学生信息', width=20,
                                            command=self.select_row)

        # 位置设置
        self.right_top_title.grid(row=1, column=0, pady=10)
        self.right_top_button1.grid(row=2, column=0, padx=10, pady=5)
        self.right_top_button2.grid(row=3, column=0, padx=10, pady=5)
        self.right_top_button3.grid(row=4, column=0, padx=10, pady=5)
        self.right_top_button4.grid(row=5, column=0, padx=10, pady=5)
    ...
    def select_row(self):
        res = messagebox.askyesnocancel('警告！', '是否查询所填数据？')
        if res == True:
            # 获取数据库的4个列表
            self.id = []
            self.name = []
            ...
            if self.var_id.get() != '' or self.var_name.get() != '' or self.var_gender.get() != '' or self.var_age.get() != '':
                db = pymysql.connect(host='localhost', port=3306, db='student2', user='root', password='123456')
                cursor = db.cursor()  # 使用cursor()方法获取操作游标
                if self.var_id.get() != '' and self.var_name.get() == '' and self.var_gender.get() == '':
                    sql1 = "SELECT * FROM student_k WHERE id = '%s'" % (self.var_id.get())  # SQadmL 查询语句
                elif self.var_id.get() == '' and self.var_name.get() != '' and self.var_gender.get() == '':
                    sql1 = "SELECT * FROM student_k WHERE name = '%s'" % (self.var_name.get())  # SQadmL 查询语句
                elif self.var_id.get() == '' and self.var_name.get() == '' and self.var_gender.get() != '':
                    sql1 = "SELECT * FROM student_k WHERE gender = '%s'" % (self.var_gender.get())  # SQadmL 查询语句
                elif self.var_id.get() != '' and self.var_name.get() != '' and self.var_gender.get() == '':
                    sql1 = "SELECT * FROM student_k WHERE id = '%s' AND name = '%s'" % (
                    self.var_id.get(), self.var_name.get())  # SQadmL 查询语句
                elif self.var_id.get() != '' and self.var_name.get() == '' and self.var_gender.get() != '':
                    sql1 = "SELECT * FROM student_k WHERE id = '%s' AND gender = '%s'" % (
                    self.var_id.get(), self.var_gender.get())
                elif self.var_id.get() == '' and self.var_name.get() != '' and self.var_gender.get() != '':
                    sql1 = "SELECT * FROM student_k WHERE name = '%s' AND gender = '%s'" % (
                ...
                messagebox.showinfo('警告！', '请填写学生数据')  # 182011011
...
class StudentView:
    def __init__(self, parent_window, student_id):
        ...
        global img
        img = tk.PhotoImage(file='picture/work3.png', )
        label_img = tk.Label(self.window, image=img)
        label_img.place(x=0, y=0, width=800, height=500)
        ...
        db = pymysql.connect(host='localhost', port=3306, db='student2', user='root', password='123456')
        cursor = db.cursor()  # 使用cursor()方法获取操作游标
        sql = "SELECT * FROM stu_view WHERE id = '%s'" % (student_id)  # SQL 查询语句
        try:
            # 执行SQL语句
            cursor.execute(sql)
            # 获取所有记录列表
        ...
        Button(self.window, text="查看成绩", width=8, font=tkFont.Font(size=16),
               command=lambda: Selectgrade(self.window, student_id)).pack(pady=10)

        Button(self.window, text="修改密码", width=8, font=tkFont.Font(size=16),
               command=lambda: Changekey(self.window)).pack(pady=10)

        Button(self.window, text="返回首页", width=8, font=tkFont.Font(size=16), command=self.back).pack(pady=10)
...
class Selectgrade:
    def __init__(self, parent_window, student_id):
        ...
        self.window = tk.Tk()  # 初始框的声明
        self.window.title('学生成绩信息')
        self.frame_center = tk.Frame(width=500, height=400)

        self.columns = ("学号", "课名", "学分", "成绩")  # 栏目4个
        self.tree = ttk.Treeview(self.frame_center, show="headings", height=18, columns=self.columns)  # 使用Treeview创建表格
        self.vbar = ttk.Scrollbar(self.frame_center, orient=VERTICAL, command=self.tree.yview)  # 设置滚动条
        # 定义树形结构与滚动条
        self.tree.configure(yscrollcommand=self.vbar.set)
        ...
        self.tree.column("课名", width=120, anchor='center')
        self.tree.column("学分", width=120, anchor='center')
        self.tree.column("成绩", width=120, anchor='center')

        # 获得总学分
        self.total = '总学分:' + ''
        db = pymysql.connect(host='localhost', port=3306, db='student2', user='root', password='123456')
        cursor = db.cursor()  # 使用cursor()方法获取操作游标
        sql2 = "CALL switch('%s')" % (student_id)  # SQL 查询语句
        try:
            # 执行SQL语句
            cursor.execute(sql2)
            # 获取所有记录列表
            results = cursor.fetchall()
            for row in results:
                self.total = '总学分:' + str(row[0])
        except:
            print("Error: unable to fetch data")
        db.close()  # 关闭数据库连接
        Label(self.window, text=self.total, font=('Verdana', 18)).grid(row=0)
        ...
        db = pymysql.connect(host='localhost', port=3306, db='student2', user='root', password='123456')
        cursor = db.cursor()  # 使用cursor()方法获取操作游标
        sql1 = "SELECT * FROM t_course WHERE SNO = '%s'" % (student_id)  # SQL 查询语句
        # 执行SQL语句
        try:
            cursor.execute(sql1)
            # 获取所有记录列表
        ...
        # 写入数据
        for i in range(len(self.SNO)):
            self.tree.insert('', i, values=(self.SNO[i], self.COURSE[i], self.CREDIT[i], self.GRADE[i]))
...
class Changekey:
    def __init__(self, parent_window):
        ...
        label = tk.Label(self.window, text='修改学生密码', bg='white', font=('Verdana', 20), height=2, pady=10)
        label.grid(row=0, column=1)

        Label(self.window, text='学号：', font=tkFont.Font(size=11)).grid(row=1, sticky=W, padx=10, pady=10)
        self.admin_id = tk.Entry(self.window)
        self.admin_id.grid(row=1, column=1, pady=10)

        Label(self.window, text='旧密码：', font=tkFont.Font(size=11)).grid(row=2, sticky=W, padx=10, pady=10)
        self.admin_key = tk.Entry(self.window, show='*')
        self.admin_key.grid(row=2, column=1, pady=10)

        Label(self.window, text='新密码：', font=tkFont.Font(size=11)).grid(row=3, sticky=W, padx=10, pady=10)
        self.admin_pass = tk.Entry(self.window, show='*')
        self.admin_pass.grid(row=3, column=1, pady=10)
        global photo
        photo = PhotoImage(file="picture/work4.png")
        Label(self.window, image=photo).grid(row=1, column=2, rowspan=3, padx=5, pady=5)

        Button(self.window, text="确定修改", width=10, font=tkFont.Font(size=15), command=self.login).grid(row=5,
                                                                                                           column=0,
                                                                                                           columnspan=2,
                                                                                                           padx=20)
        Button(self.window, text="返回首页", width=10, font=tkFont.Font(size=15), command=self.back).grid(row=5,
                                                                                                          column=1,
                                                                                                          columnspan=2,
                                                                                                          padx=20)
...
class AboutPage:
    def __init__(self, parent_window):
        ...
        img3 = tk.PhotoImage(file='picture/work3.png', )
        label_img3 = tk.Label(self.window, image=img3)
        label_img3.place(x=0, y=0, width=692, height=461)

        self.window.title('账号系统')
        self.window.geometry('400x400')

        label = tk.Label(self.window, text='账号系统', font=('Verdana', 20), height=2)
        label.pack()

        Label(self.window, text='学生账号为学号', font=('Verdana', 18)).pack(pady=5)
        Label(self.window, text='初始密码为123456', font=('Verdana', 18)).pack(pady=5)

        Button(self.window, text="修改密码", font=tkFont.Font(size=16),
               command=lambda: Changekey(self.window)).pack(pady=15)
        Button(self.window, text="管理员修改", font=tkFont.Font(size=16),
               command=lambda: Adminlogin(self.window)).pack(pady=15)
        Button(self.window, text="返回首页", font=tkFont.Font(size=16), command=self.back).pack(pady=15)
...
class Adminlogin:
    def __init__(self, parent_window):
        ...
        photo = PhotoImage(file="picture/work4.png")
        Label(self.window, image=photo).grid(row=1, column=2, rowspan=2, padx=5, pady=5)

        Button(self.window, text="修改密码", width=8, font=tkFont.Font(size=12), command=self.login_change).grid(row=3,
                                                                                                                 column=0,
                                                                                                                 pady=20)
        Button(self.window, text="创建账号", width=8, font=tkFont.Font(size=12), command=self.login_create).grid(row=3,
                                                                                                                 column=1,
                                                                                                                 pady=20)
        Button(self.window, text="返回首页", width=8, font=tkFont.Font(size=12), command=self.back).grid(row=3,
                                                                                                         column=2,
                                                                                                         pady=20)
...
class CreateAdminPage:
    ...
    def find(self):
        ...
        db = pymysql.connect(host='localhost', port=3306, db='student2', user='root', password='123456')  # 打开数据库连接
        cursor = db.cursor()  # 使用cursor()方法获取操作游标
        sql = "SELECT admin_id FROM admin_login_k WHERE admin_id = '%s'" % (self.admin_id.get())  # SQL 查询语句
        try:
            # 执行SQL语句
            cursor.execute(sql)
            # 获取所有记录列表
