...
void divx(u8 *s1,u8 *s2,u8 out1[])
{
   ...
   if(com(s1,s2)==0){
	...
	{	
		   i=0;while(s1[i]=='0')i++;j=0;              //除去首位0
			 for(;i<strlen(s1);i++)an1[j++]=s1[i];
		   
			 while(com(an1,s2)>=0)
				 {
					  memset(an2,0,MAXN);
					  i=0;while(s2[i]=='0')i++;j=0;             //除去首位0
...
void dat_add(u8 symb0,u8 pot0,u8 s0[],u8 symb1,u8 pot1,u8 s1[],u8 out[])       //符号，小数位数，数据字符串   symb=0 为正数 反之为负数
{
	...
	if(symb0 == symb1)
	{
		...
		{
			len = strlen(s1);
			for(i=0;i<pot0-pot1;i++) s1[len+i] = '0';	               //小数点对齐
			addx(s0,s1,sout);
			
			len = strlen(sout); 
		
			for(i=0;i<len-pot0;i++)sout1[i]=sout[i];                       //小数点前的数
			sout1[len-pot0]='.';                                       //添加小数点
			for(i=len-pot0;i<len;i++)sout1[i+1]=sout[i];                //小数点后的数
		}
		else if(pot1>pot0)
		{
			len = strlen(s0);
			for(i=0;i<pot1-pot0;i++) s0[len+i] = '0';	               //小数点对齐
			addx(s0,s1,sout);
			
			len = strlen(sout);                                     
			for(i=0;i<len-pot1;i++)sout1[i]=sout[i];                       //小数点前的数
			sout1[len-pot1]='.';                                       //添加小数点
			for(i=len-pot1;i<len;i++)sout1[i+1]=sout[i];                //小数点后的数
		}
	...
	else if(symb0>symb1)  //第一个为负数
		{
			if(pot0==0 && pot1==0)
			{
					if(com(s0,s1)==1)         //第一个的数值大于第二个
					{
						subx(s0,s1,sout1);
						len = strlen(sout1);	
						out[0]='-';   
	...
	else if(symb0<symb1)  //第2个为负数
		{
			if(pot0==0 && pot1==0)
			{
				if(com(s0,s1)>=0)         //第一个的数值大于第二个
				{
					subx(s0,s1,sout1);
					len = strlen(sout1);
					for(i=0;i<len;i++) out[i]=sout1[i];
...
void dat_sub(u8 symb0,u8 pot0,u8 s0[],u8 symb1,u8 pot1,u8 s1[],u8 out[])       //符号，小数位数，数据字符串   symb=0 为正数 反之为负数	
{
//	//////////////////////////////////////////
//	
//	//  a - b = a + -b                  //减法调用加法定律
...
void dat_mul(u8 symb0,u8 pot0,u8 s0[],u8 symb1,u8 pot1,u8 s1[],u8 out[])       //符号，小数位数，数据字符串   symb=0 为正数 反之为负数	
{
	char i;
	u8 len;
	u8 sout[MAXN*2]=0;
	u8 sout1[MAXN*2]=0;
		...
		else
		{
			...
			for(i=0;i<len-pot0-pot1;i++)sout1[i]=sout[i];                       //小数点前的数
			sout1[len-pot0-pot1]='.';                                       //添加小数点
			for(i=len-pot0-pot1;i<len;i++)sout1[i+1]=sout[i];                //小数点后的数
...
void dat_div(u8 symb0,u8 pot0,u8 s0[],u8 symb1,u8 pot1,u8 s1[],u8 out[])
{
	...
	strcat(sout,ss);                         //字符串前面添加0，15位长度对齐
...
void main()
{
	char key=0;   //按键值
	u8 i;
  u8	cnt0 = 0,cnt1 = 0;          //用于显示的数据长度
	u8 dat_cnt0=0,dat_cnt1=0;      //用于计算的数据长度
	u8 buf[MAXN]={0};

	u8 dat_flag = 0;
	u8 symbol=0;   //运算符号
	
	u8 pot0_flag = 0;           //小数点位置
	u8 pot1_flag = 0;
...
//	LCD_ShowStr(1,buf);
	...
	while(1)
	{
		...
		if(key!=-1)          //有按键按下
		{
									...
									if(key<12) //按键值为 0123456789.
									{
												if(dat_flag==0)
												{
													      ...
													      if(key == fuh && cnt0==0)
																...
																{
																				if(strstr(show0,".")==0 && cnt0!=0)     //有小数点并不是数据第一位
																				{
																					show0[cnt0++] = '.';
																				}
																}
												...
												else
												{
													      if(key == fuh && cnt1==0)
																...
																{
																				if(strstr(show1,".")==0 && cnt1!=0)     //有小数点并不是数据第一位
																				{
																					show1[cnt1++] = '.';
																				}
																}
									...
									else if(key>12 && dat_cnt0>0)              //运算符号按下并且数据0有数据
									{
													symbol = key;
										     switch(symbol)
												 {
